...
from src.configuration import WORD_NUMERIC_VECTOR_SIZE, EPOCHS_NUMBER, DROPOUT, RECURRENT_DROPOUT, \
    TIME_STEP, get_network_model_snapshot, DATA_SET, get_csv_log_file_name
from src.preprocessing.document_as_w2v_groups import ensure_word_numeric_representation_created
from src.utils.data_generator import get_train_generator, get_test_generator
...
model = Sequential()
model.add(
    LSTM(200, input_shape=(TIME_STEP, WORD_NUMERIC_VECTOR_SIZE), dropout=DROPOUT, recurrent_dropout=RECURRENT_DROPOUT))
...
model.fit_generator(get_train_generator(),
                    ...
                    samples_per_epoch=int(50000 * 0.7) // TIME_STEP,
                    callbacks=callbacks,
                    validation_steps=int(50000 * 0.3) // TIME_STEP)
